...
class TestSceneObject:
    def test_valid_object(self) -> None:
        obj = SceneObject(name="table", x=0.0, y=0.0, z=0.0,
    ...
    def test_name_spaces_to_underscores(self) -> None:
        obj = SceneObject(name="office chair", x=0.0, y=0.0, z=0.0,
                          rot_x=0.0, rot_y=0.0, rot_z=0.0)
        assert obj.name == "office_chair"

    def test_string_numerics_coerced(self) -> None:
        obj = SceneObject(name="lamp", x="1.5", y="-0.8", z="0.0",  # type: ignore[arg-type]
                          rot_x="0.0", rot_y="0.0", rot_z="0.785")  # type: ignore[arg-type]
        assert obj.x == pytest.approx(1.5)
        assert obj.rot_z == pytest.approx(0.785)

    def test_invalid_name_raises(self) -> None:
        with pytest.raises(Exception):
            SceneObject(name="", x=0.0, y=0.0, z=0.0,
                        rot_x=0.0, rot_y=0.0, rot_z=0.0)

    def test_negative_scale_raises(self) -> None:
        with pytest.raises(Exception):
            SceneObject(name="table", x=0.0, y=0.0, z=0.0,
                        rot_x=0.0, rot_y=0.0, rot_z=0.0, scale=-1.0)
