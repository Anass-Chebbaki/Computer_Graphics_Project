...
class TestOBBExtended:
    def test_intersects_touching_boundary(self) -> None:
        """Due OBB che si toccano esattamente sul bordo AABB."""
        a = OBB(cx=0.0, cy=0.0, half_w=1.0, half_d=1.0)
        b = OBB(cx=2.0, cy=0.0, half_w=1.0, half_d=1.0)
    ...
    def test_y_direction_separation(self) -> None:
        ...
        a = OBB(cx=0.0, cy=0.0, half_w=0.5, half_d=0.5)
        b = OBB(cx=0.0, cy=5.0, half_w=0.5, half_d=0.5)
        assert a.intersects(b) is False

    def test_area_with_zero_dimensions(self) -> None:
        """Area con half_w=0 e half_d=0."""
        bbox = OBB(cx=0.0, cy=0.0, half_w=0.0, half_d=0.0)
...
class TestOBBComprehensiveMock:
    def test_obb_initialization(self) -> None:
        """Test OBB creation with valid parameters."""
        bbox = OBB(cx=1.0, cy=2.0, half_w=1.5, half_d=2.0, angle=0.0)
        assert bbox.cx == 1.0
        assert bbox.cy == 2.0
        assert bbox.half_w == 1.5
        assert bbox.half_d == 2.0

    def test_obb_properties(self) -> None:
        """Test OBB computed properties (AABB fallback)."""
        bbox = OBB(cx=1.0, cy=2.0, half_w=1.0, half_d=1.5, angle=0.0)
        assert bbox.x_min == 0.0
        assert bbox.x_max == 2.0
        assert bbox.y_min == 0.5
        assert bbox.y_max == 3.5

    def test_obb_intersects_true(self) -> None:
        """Test intersection detection when boxes overlap."""
        bbox1 = OBB(cx=1.0, cy=1.0, half_w=1.0, half_d=1.0)
        bbox2 = OBB(cx=1.5, cy=1.5, half_w=1.0, half_d=1.0)
        assert bbox1.intersects(bbox2) is True

    def test_obb_intersects_false(self) -> None:
        """Test intersection detection when boxes don't overlap."""
        bbox1 = OBB(cx=0.0, cy=0.0, half_w=0.5, half_d=0.5)
        bbox2 = OBB(cx=5.0, cy=5.0, half_w=0.5, half_d=0.5)
        assert bbox1.intersects(bbox2) is False

    def test_obb_area(self) -> None:
        """Test area calculation."""
        bbox = OBB(cx=0.0, cy=0.0, half_w=2.0, half_d=3.0)
...
class TestMeshParsing:
    def test_parse_obj_dimensions(self, tmp_path: Path) -> None:
        """Test parsing dimensions from a mock OBJ file."""
        obj_content = "v 1.0 1.0 1.0\n" "v -1.0 -1.0 -1.0\n" "f 1 2 3\n"
        obj_file = tmp_path / "test.obj"
        obj_file.write_text(obj_content)

        from computer_graphics.scene_graph import _parse_obj_dimensions

        dims = _parse_obj_dimensions(obj_file)
        assert dims == (2.0, 2.0, 2.0)

    def test_parse_obj_empty(self, tmp_path: Path) -> None:
        obj_file = tmp_path / "empty.obj"
        obj_file.write_text("g background\n")
        from computer_graphics.scene_graph import _parse_obj_dimensions

        assert _parse_obj_dimensions(obj_file) is None

    def test_parse_glb_dimensions(self, tmp_path: Path) -> None:
        """Test parsing dimensions from a mock GLB file."""
        import json
        import struct

        # Mock GLB chunk 0 (JSON)
        gltf_json = {
            "accessors": [
                {"type": "VEC3", "min": [-1.0, -2.0, -3.0], "max": [1.0, 2.0, 3.0]}
            ]
        }
        ...
        header = (
            b"glTF" + struct.pack("<I", 2) + struct.pack("<I", 12 + 8 + len(json_data))
        )
        # Chunk 0: length(4), type(4), data
        chunk0 = struct.pack("<I", len(json_data)) + b"JSON" + json_data

        glb_file = tmp_path / "test.glb"
        glb_file.write_bytes(header + chunk0)

        from computer_graphics.scene_graph import _parse_glb_dimensions

        dims = _parse_glb_dimensions(glb_file)
        assert dims == (2.0, 4.0, 6.0)

    def test_get_asset_dimensions_with_mesh(self, tmp_path: Path) -> None:
        """Test get_asset_dimensions prioritizing mesh parsing."""
        (tmp_path / "table.obj").write_text("v 0 0 0\nv 1 1 1\n")
        from computer_graphics.scene_graph import (
            _mesh_dimensions_cache,
